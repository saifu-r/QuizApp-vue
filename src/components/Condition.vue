  ...
  <div class="App">
    ...
    <div class="container2" v-if="isAvailable">
      ...
      <input
        ...
        v-model="answer"
        @change="checkanswer"
        v-if="!radioinvisible && isPrevious === false"
      />{{ answer_a }}
      <br />
      <input
        type="radio"
      ...
      <input
        ...
        v-model="answer"
        @change="checkanswer"
        v-if="answer_c !== null && !radioinvisible && isPrevious === false"
      />{{ answer_c }}
      <br />
      <input
        type="radio"
        ...
        v-model="answer"
        @change="checkanswer"
        v-if="answer_d !== null && !radioinvisible && isPrevious === false"
      />{{ answer_d }}
      <br />
      <input
        type="radio"
        ...
        v-model="answer"
        @change="checkanswer"
        v-if="answer_e !== null && !radioinvisible && isPrevious === false"
      />{{ answer_e }}
      <br />
      <br /><br />
      <!-- <h6>{{ answer }}</h6> -->
      ...
      <div style="display: flex; justify-content: space-between">
        <button @click="previous" :disabled="countNext === 0" style="order: 1">
          Previous
        </button>
        <button v-if="limit === countNext + 1" style="order: 2">Submit</button>
        <button
          @click="next"
          :disabled="limit <= countNext + 1"
          style="order: 3"
        >
          Next
        </button>
...
export default defineComponent({
  ...
  setup() {
    ...
    const showquestionanswer = (term: number) => {
      ...
      correctanswer_a.value =
        quizData.value[term].correct_answers.answer_a_correct; //3.correct_answers.answer_a_correct
      correctanswer_b.value =
        quizData.value[term].correct_answers.answer_b_correct;
      correctanswer_c.value =
        quizData.value[term].correct_answers.answer_c_correct;
      correctanswer_d.value =
        quizData.value[term].correct_answers.answer_d_correct;
      correctanswer_e.value =
        quizData.value[term].correct_answers.answer_e_correct;
...
.container1 {
  ...
  background-color: #f4f4f2;
  color: #27374d;
  box-shadow: 6px 6px 6px rgb(172, 171, 171);
}
